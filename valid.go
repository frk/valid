// NOTE(mkopriva): DO NOT REMOVE THE "valid:rule.yaml" DIRECTIVES AND THE YAML THAT
// FOLLOWS AFTER THEM. The directives & yaml in the functions' comments are required
// by the cmd/internal/rules parser to resolve the properties of a rule's function.
...
import (
	...
	"github.com/frk/valid/internal/l10n"
...
// valid:rule.yaml
//	name: ein
//	error: { text: "must be a valid EIN" }
func todo_EIN(v string) bool {
	// TODO
	return false
}
...
// valid:rule.yaml
//	name: ic
//	error: { text: "must be a valid identity card number" }
func todo_IC(v string) bool {
	// TODO
	return false
}
...
func ISO639(v string, num int) bool {
	...
	if num == 1 && len(v) == 2 {
		v = strings.ToLower(v)
		_, ok := tables.ISO_639_1[v]
		return ok
	}

	if num == 2 && len(v) == 3 {
		v = strings.ToLower(v)
		_, ok := tables.ISO_639_2[v]
		return ok
	}
...
func ISO31661A(v string, num int) bool {
	...
	if num == 2 && len(v) == 2 {
		v = strings.ToUpper(v)
		_, ok := l10n.ISO31661A_2[v]
		return ok
	}

	if num == 3 && len(v) == 3 {
		v = strings.ToUpper(v)
		_, ok := l10n.ISO31661A_3[v]
		return ok
	}
...
// valid:rule.yaml
//	name: passport
//	error: { text: "must be a valid passport number" }
func todo_PassportNumber(v string) bool {
	// TODO
	return false
}
...
func Phone(v string, cc string) bool {
	if c, ok := l10n.Get(cc); ok && c.Phone != nil {
...
// valid:rule.yaml
//	name: url
//	error: { text: "must be a valid URL" }
func todo_URL(v string) bool {
...
func Zip(v string, cc string) bool {
	if c, ok := l10n.Get(cc); ok && c.Zip != nil {
