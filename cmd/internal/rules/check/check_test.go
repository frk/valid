...
func TestMain(m *testing.M) {
	...
	if err := spec.LoadIncludedSpecs(&test_ast); err != nil {
		log.Fatal(err)
	}
...
func TestCheckerCheck(t *testing.T) {
	mypkg := types.Pkg{
		Path: "github.com/frk/valid/cmd/internal/rules/check/testdata/mypkg",
		Name: "mypkg",
	}
	timepkg := types.Pkg{
		Path: "time",
	...
	groups := []struct {
		...
		tests: []testCase{{
			...
			err: &Error{C: E_OPTIONAL_CONFLICT,
				...
				ty: T.Ptr(T.string),
				r:  &rules.Rule{Name: "optional"},
				rs: spec.Get("optional"),
				r2: &rules.Rule{Name: "required"},
				//rs: spec.Get("required"),
			},
		}, {
	...
	if err := spec.LoadCustomSpecs(cfg, &test_ast); err != nil {
...
var T = test_values{
	...
	pkg: types.Pkg{
		Path: "github.com/frk/valid/cmd/internal/rules/check/testdata",
		Name: "testdata",
	},
}

func init() {
	spec.AddCustom("pre:p1", &spec.Spec{
		Name: "p1", Kind: spec.PREPROC,
		FName: "MyPreproc1",
		FType: &types.Type{
			Pkg: types.Pkg{
				Name: "mypkg",
	...
	spec.AddCustom("pre:p2", &spec.Spec{
		Name: "p2", Kind: spec.PREPROC,
		FName: "MyPreproc2",
		FType: &types.Type{
			Pkg: types.Pkg{
				Name: "mypkg",
	...
	spec.AddCustom("pre:p4", &spec.Spec{
		Name: "p4", Kind: spec.PREPROC,
		FName: "MyPreproc4",
		FType: &types.Type{
			Pkg: types.Pkg{
				Name: "mypkg",
	...
	spec.AddCustom("pre:p0", &spec.Spec{
		Name: "p0", Kind: spec.FUNCTION,
		FName: "MyPreproc0",
		FType: &types.Type{
			Pkg: types.Pkg{
				Name: "mypkg",
