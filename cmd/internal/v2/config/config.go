...
func (c *Config) normalizeAndCheck() (err error) {
	...
	// check custom types
	seen = make(map[string]bool) // to ensure uniqueness
	for i, tc := range c.Types {
		if tc.Type.Name == "" {
			return &Error{C: ERR_TYPE_NONAME, dir: c.WorkDir.Value,
				file: c.File.Value, val: strconv.Itoa(i)}
		}

		typeKey := tc.Type.String()

		// make sure types are unique
		if seen[typeKey] {
			return &Error{C: ERR_TYPE_DUPNAME, dir: c.WorkDir.Value,
				file: c.File.Value, val: typeKey}
		}
		seen[typeKey] = true
	}
	return nil
