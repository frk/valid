...
func (T *Type) IsPtrOf(U *Type) bool {
	return T.Kind == PTR && T.Elem.IsIdenticalTo(U)
}
...
func (T *Type) IsIdenticalTo(U *Type) bool {
	...
	case PTR:
		...
		return T.Elem.IsIdenticalTo(U.Elem)
	case ARRAY:
		// Two array types are identical if they have identical
		// element types and the same array length.
		return T.ArrayLen == U.ArrayLen && T.Elem.IsIdenticalTo(U.Elem)
	case SLICE:
		// Two slice types are identical if they have identical element types.
		return T.Elem.IsIdenticalTo(U.Elem)
	case MAP:
		// Two map types are identical if they have identical key and value types.
		return T.Key.IsIdenticalTo(U.Key) && T.Elem.IsIdenticalTo(U.Elem)
	case CHAN:
	...
	case STRUCT:
		...
		for i, f := range T.Fields {
			...
			if !f.Type.IsIdenticalTo(g.Type) {
				return false
			}
		}
		return true
