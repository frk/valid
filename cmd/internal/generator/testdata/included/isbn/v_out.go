// DO NOT EDIT. This file was generated by "github.com/frk/valid".

package testdata

import (
	"errors"

	"github.com/frk/valid"
)

func (v Validator) Validate() error {
	if !valid.ISBN(v.F1, 0) {
		return errors.New("F1 must be a valid ISBN")
	}
	if v.F2 != nil && !valid.ISBN(*v.F2, 0) {
		return errors.New("F2 must be a valid ISBN")
	}
	if !valid.ISBN(v.F3, 10) {
		return errors.New("F3 must be a valid ISBN")
	}
	if v.F4 != nil && !valid.ISBN(*v.F4, 13) {
		return errors.New("F4 must be a valid ISBN")
	}
	return nil
}
