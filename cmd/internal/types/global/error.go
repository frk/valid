...
const (
	...
	E_OBJECT_SEARCH            // searching for object failed
	E_ERROR_CONSTRUCTOR_OBJECT // error constructor object not *types.Func
	E_ERROR_CONSTRUCTOR_TYPE   // error constructor func does not have correct signature
	E_ERROR_AGGREGATOR_OBJECT  // error aggregator object not *types.TypeName
	E_ERROR_AGGREGATOR_TYPE    // error aggregator type does not implement interface
)
...
var error_template = `
{{ define "` + E_OBJECT_SEARCH.ident() + `" -}}
{{.OriginalError -}}
{{ end }}
{{ define "` + E_ERROR_CONSTRUCTOR_OBJECT.ident() + `" -}}
{{ ERROR }} Cannot use "{{W .ObjId}}" (object {{.Object}}) as a global error constructor func.
{{ end }}
{{ define "` + E_ERROR_CONSTRUCTOR_TYPE.ident() + `" -}}
...
{{ end }}
{{ define "` + E_ERROR_AGGREGATOR_OBJECT.ident() + `" -}}
{{ ERROR }} Cannot use "{{W .ObjId}}" (object {{.Object}}) as a global error aggregator type.
{{ end }}
{{ define "` + E_ERROR_AGGREGATOR_TYPE.ident() + `" -}}
