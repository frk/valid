...
func TestAnalyzeValidator(t *testing.T) {
	...
	tests := []struct {
		...
		want: &Validator{
			Type: &Type{
				...
				Fields: []*StructField{{
					Pkg:  pkg0,
					Name: "err",
					Obj: &Obj{Type: &Type{Pkg: pkg0, Name: "errorConstructor", Kind: STRUCT, MethodSet: []*Method{{
						Pkg:  pkg0,
						Name: "Error",
						Type: &Type{
							Kind:       FUNC,
							...
							In: []*Var{
								...
								{Name: "args", Type: &Type{Kind: SLICE, Elem: &Obj{Type: &Type{Kind: INTERFACE}}}},
							},
		...
		want: &Validator{
			Type: &Type{
				...
				Fields: []*StructField{{
					Pkg:  pkg0,
					Name: "err",
					Obj: &Obj{Type: &Type{Pkg: pkg0, Name: "errorAggregator", Kind: STRUCT, MethodSet: []*Method{{
						Pkg:  pkg0,
						Name: "Error",
						Type: &Type{
							Kind:       FUNC,
		...
		// (*types.Validator).Type.MethodSet[0].Type.Out[0].Type.MethodSet[0].Pkg.Name
