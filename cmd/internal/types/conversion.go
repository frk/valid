...
func (V *Type) IsConvertibleTo(T *Type) bool {
	...
	if V.Kind == PTR && T.Kind == PTR && !V.IsNamed() && !T.IsNamed() {
		Vubase := V.Elem.Type.Underlying()
		Tubase := T.Elem.Type.Underlying()
	...
	if V.Kind == SLICE {
		switch T.Kind {
		case ARRAY:
			if V.Elem.Type.IsIdenticalTo(T.Elem.Type) {
				// NOTE: conversion of slices to arrays requires go1.20 or later
				return true
			}
		case PTR:
			if A := T.Elem.Type; A.Kind == ARRAY {
				if V.Elem.Type.IsIdenticalTo(A.Elem.Type) {
